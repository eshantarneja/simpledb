...
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
		...
		// some code goes here
			...
			if(pid.pgcateg() == BTreePageId.LEAF) {
				...
				BTreeInternalPage curr = (BTreeInternalPage) this.getPage(tid, dirtypages, pid, Permissions.READ_ONLY);
				Iterator<BTreeEntry> currIter = curr.iterator();
				BTreeEntry currEntry = null;
				while(currIter.hasNext()) {
					currEntry = currIter.next();
					if (f == null||f.compare(Op.LESS_THAN_OR_EQ, currEntry.getKey())) {
	...
	protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field) 
		...
		// the newLeaf.getNumTuples() != new entry.  getParentWithEmtpySlots() will be useful here.  Don't forget to update
		// the sibling pointers of all the affected leaf pages.  Return the page into which a 
		// tuple with the given key field should be inserted.
		
		//copy half 
		BTreeLeafPage newLeaf = (BTreeLeafPage)getEmptyPage(tid, dirtypages, BTreePageId.LEAF);
		Iterator<Tuple> oldIter = page.reverseIterator();
		//what if it is an odd number??
		int half = (int)Math.ceil((page.getNumTuples())/2);
		Tuple currTup = null;
		for(int i = 0; i < half; i++) {
			if(oldIter.hasNext()) {
				currTup = oldIter.next();
				page.deleteTuple(currTup);
				newLeaf.insertTuple(currTup);							
			}							
		}
		//update siblings (new page is on right of old page)
		...
		if(rightId != null) {
			BTreeLeafPage rightSib = (BTreeLeafPage) this.getPage(tid, dirtypages, rightId, Permissions.READ_WRITE);
			rightSib.setLeftSiblingId(newLeaf.getId());
		}
		
		page.setRightSiblingId(newLeaf.getId());
		
		//copy up middle value		
		Field midField = currTup.getField(keyField);
		BTreeInternalPage parPage = this.getParentWithEmptySlots(tid, dirtypages, page.getParentId(), field);
		BTreeEntry middle = new BTreeEntry(midField, page.getId(), newLeaf.getId());
		parPage.insertEntry(middle);
		this.updateParentPointers(tid, dirtypages, parPage);
		
		//check to see which of the pages your inserted field will be in and return
	...
	protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		Iterator<BTreeEntry> oldIter = page.reverseIterator();

		int half = (int)Math.floor((page.getNumEntries())/2);
		BTreeEntry currEntry = null;
		for(int i = 0; i < half; i++) {
			if(oldIter.hasNext()) {
				currEntry = oldIter.next();
				page.deleteKeyAndRightChild(currEntry);
				newPage.insertEntry(currEntry);						
			}							
		}
	
		//push*** up middle value
		BTreeEntry middle = oldIter.next();
		...
		if(field.compare(Op.GREATER_THAN_OR_EQ, middle.getKey())){
			return newPage;
		}
		else {
			return page;
